...
import { parseTmdlFolder } from './tmdlParser.js'
...
function send(res, status, body, headers = {}) {
  ...
  res.writeHead(status, { 'Content-Type': isBuffer ? 'application/octet-stream' : typeof body === 'string' ? 'text/plain; charset=utf-8' : 'application/json; charset=utf-8', ...headers })
  res.end(payload)
}
...
async function readBody(req, limit = 80 * 1024 * 1024) {
  ...
  for await (const chunk of req) {
    size += chunk.length
    if (size > limit) throw new Error('Upload exceeds 80 MB limit')
    chunks.push(chunk)
  }
...
async function json(req) {
  const b = await readBody(req, 15 * 1024 * 1024)
...
function cleanFileName(name) { return path.basename(name || 'semantic-model.zip').replace(/[^a-zA-Z0-9._-]/g, '_') }
